...
export function Rosca({ fatias, total, rotuloCentro }) {
  ...
  return (
    <svg viewBox="0 0 192 192" style={{ width: '100%', maxWidth: 200, display: 'block', margin: '0 auto' }}
      ...
      {partes}
      <text x="96" y="90" textAnchor="middle" fontFamily="ui-monospace, Menlo, monospace" fontSize="8.5"
        letterSpacing="1.4" fill="var(--tinta-3)">{rotuloCentro || 'MAIOR CLASSE'}</text>
      <text x="96" y="106" textAnchor="middle" fontFamily="Iowan Old Style, Palatino, Georgia, serif"
        fontSize="15" fontWeight="600" fill="var(--tinta)">{maior.chave}</text>
      <text x="96" y="120" textAnchor="middle" fontFamily="ui-monospace, Menlo, monospace" fontSize="10"
...
export function Colunas({ dados, formatar }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto' }} role="img" aria-label="Série mensal">
      ...
      <text x={W - pad} y={yMedia - 5} textAnchor="end" fontFamily="ui-monospace, Menlo, monospace"
      ...
      {dados.map((d, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x + w / 2} y={H - 9} textAnchor="middle" fontFamily="ui-monospace, Menlo, monospace"
...
export const Ponto = ({ classe }) => (
  <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: 2,
    background: corClasse(classe), marginRight: 6 }} />
)
